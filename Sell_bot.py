...
# Set up gas prices and limits
GAS_PRICE = w3.toWei('5', 'gwei')
GAS_LIMIT = 1000000

# Set up PancakeSwap Factory contract
...
# Set up PancakeSwap Router contract
router_contract = w3.eth.contract(address=PANCAKESWAP_ROUTER_ADDRESS, abi=ROUTER_ABI)

def getPair(token_address):
    pair_address = factory_contract.functions.getPair(token_address, router_contract.functions.WETH().call()).call()
    return pair_address

while True:
    liquidity = getPair(TOKEN_ADDRESS)
    if liquidity:
        
        
        token_contract = w3.eth.contract(address=TOKEN_ADDRESS, abi=TOKEN_ABI)
        amount_in = token_contract.functions.balanceOf(WALLET_ADDRESS).call()
        tokenSymbol = token_contract.functions.symbol().call()
        decimals = token_contract.functions.decimals().call() 

        # Approve the PancakeSwap Router to spend the token
        print(f'🟩🟩🟩🟩🟩🟩 Amazing ! Liquidity has been detected for {tokenSymbol} on PancakeSwap 💧 ')


        if amount_in <= 0:
        	print(f"🟥🟥🟥🟥🟥🟥 Your {tokenSymbol} balance is 0, you can't sell 0 token we stop here bro. You are so poor 😳")
        	continue
        else:
        	print(f'🟦🟦🟦🟦🟦🟦 Nice you have {amount_in / 10**decimals} {tokenSymbol} go sell it fast now 🗿')
        
        print('🟧🟧🟧🟧🟧🟧 I ask to approve the token, so PancakeSwap can give your fucking BNB back 🫡')
        tx_hash = token_contract.functions.approve(PANCAKESWAP_ROUTER_ADDRESS, 2**256-1).buildTransaction({
            'from': WALLET_ADDRESS,
            'gasPrice': GAS_PRICE,
            'gas': GAS_LIMIT,
        ...
        # Sell the token for WBNB
        print(f'🟧🟧🟧🟧🟧🟧 Now get the fuck out of the wallet {tokenSymbol} 🏋🏻‍♂️ ')

        spend = w3.toChecksumAddress("0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c")  #WBNB
        ...
        if amount_in > 0:
            ...
            try:
                ...
                tx_receipt = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
                time.sleep(1)
                print(f'🟩🟩🟩🟩🟩🟩 Bro you get your BNB back, {tokenSymbol} are now out congrat 🫡 ' + w3.toHex(tx_receipt))
            except:
                print("🟥🟥🟥🟥🟥🟥 Error: Problem when selling token blow for our friend 😢")
            break
        else:
            print(f"🟥🟥🟥🟥🟥🟥 Your {tokenSymbol} balance is 0, you can't sell 0 token we stop here bro. You are so poor 😳")
            break
    else:
    	print(f'🟧🟧🟧🟧🟧🟧 Waiting for liquidity to sell this crap token 👀')
    	time.sleep(1)
